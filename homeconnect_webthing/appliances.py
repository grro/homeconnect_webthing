    ...
    def write_start_date(self, start_date: str):
        ...
        if len(self._program_selected) == 0:
            ...
            if remaining_secs_to_wait >= self.__program_start_in_relative_sec_max:
                logging.warning("remaining seconds to wait " + print_duration(remaining_secs_to_wait) + " larger than " + print_duration(self.__program_start_in_relative_sec_max) + ". Ignore setting start date")
    ...
    def write_start_date(self, start_date: str):
        ...
        if self.state == self.STATE_STARTABLE:
            ...
            if remaining_secs_to_finish >= self.__program_finish_in_relative_max_sec:
                logging.warning("remaining seconds to finished " + print_duration(remaining_secs_to_finish) + " larger than " + print_duration(self.__program_finish_in_relative_max_sec) + ". Ignore setting start date")
            else:
